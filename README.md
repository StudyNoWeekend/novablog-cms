...
**CORS 白名单**以数据库为唯一权威数据源：

1. 数据库中持久化的配置（后台「个人资料 → 跨域配置」页面可编辑）——一经保存，不被任何来源覆盖，修改后即时生效、重启后仍生效
2. `config.yaml` 中的 `cors.allowed_origins`——仅在数据库无有效数据时作为初始值播种（数据库已有后台保存过的配置时不再回读）
3. 程序内置默认值 `http://localhost:5173,http://localhost:5174`（config.yaml 也未配置时的最终兜底）
